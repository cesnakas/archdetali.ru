...
<?php
//file_put_contents("./log1.txt",print_r($_POST,true));
...
if($_POST['MESSAGE'] and $_POST['MESSAGE'] !="") {$message = $_POST['MESSAGE'];}
...
if( isset($_GET['id']) && isset($_POST['PHONE']) && isset($_POST['AJAX_CALL']) )
  if( ($_POST['AJAX_CALL'] == 'Y') && ($_GET['id'] == 67))
   {
@include "classes/ActionController.php";
	   //file_put_contents("./form-67.log.txt","starting ...");
	   ...
	   //  file_put_contents("./forma-zayavka.log.txt",print_r($data,1));
				$newOrder = new ActionController($data);
                $newOrder->keepname=1;
                $newOrder->mailsubjecttext="Заявка с сайта Архдетали, Консультация";

           $test = $newOrder->sendMailForm('заявка с сайта Архдетали, консультация','Заявка на консультацию' );
...
if(isset($_POST['phone'])  ) {
		...
		if($_POST['form_id'] == 2) {
			...
			if($notags == 0) {
				...
				//file_put_contents("./log2.txt","Before send mail");
				$newOrder = new ActionController($data);
				//Отправка Битрикс
				//$test = $newOrder->sendBitrix('заявка с сайта Дизайнквартир.рф', $title);
                $newOrder->mailsubjecttext="Заявка с сайта Архдетали, калькулятор";

                $test = $newOrder->sendMailForm('заявка с сайта Архдетали, калькулятор', iconv("UTF-8","WINDOWS-1251",$message) );
