...
if($arResult["ITEMS"]){?>
	...
	"AREA_FILE_SHOW" => "file",
		"PATH" => SITE_DIR."include/front-filter.php",
							...
							<div class="bx_filter_parameters_box_title" >
								<span>
									<?if(strlen($arItem['FILTER_HINT'])):?>
										<span data-html="true" data-toggle="tooltip" data-delay='{"show":"200", "hide":"500"}' data-original-title="<?=str_replace('"', "'", $arItem['FILTER_HINT'])?>" rel="tooltip"><?=$arItem["NAME"]?></span>
									<?else:?>
										<?=$arItem["NAME"]?>
									<?endif;?>
								</span>
							...
							switch ($arItem["DISPLAY_TYPE"]){
								...
								case "P"://DROPDOWN
									...
									<div class="bx_filter_select_container">
										<div class="bx_filter_select_block" data-id="<?=CUtil::JSEscape($key)?>" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
							...
							<div class="bx_filter_block">
								<div class="bx_filter_parameters_box_container">
									<div class="bx_filter_popup_result right" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
										...
										<!-- noindex --><a href="<?echo $arParams["FORM_URL"]?>" class="button white_bg" rel="nofollow"><?echo GetMessage("CT_BCSF_FILTER_SHOW")?></a><!-- /noindex -->
									</div>
	...
	<script>
		var smartFilter = new JCSmartFilter('<?echo CUtil::JSEscape($arParams["FORM_URL"])?>', '<?=$arParams["VIEW_MODE"];?>', <?=CUtil::PhpToJSObject($arResult["JS_FILTER_PARAMS"])?>);
		<?if(!$isFilter){?>
