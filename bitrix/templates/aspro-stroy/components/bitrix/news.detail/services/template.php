...
<?php
// Ïåðåìåííàÿ ôîðìû. #FORM_ID_1# ÇÀÌÅÍßÅÒ ÍÀ ÔÎÐÌÓ
@require $_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/aspro-stroy/include_areas/form1.php";
$arResult['FIELDS']['DETAIL_TEXT'] = preg_replace("/#FORM_ID_([\d]+)#/is", $form1, $arResult['FIELDS']['DETAIL_TEXT']);
?>
<?// element name?>
...
<?if($arResult['FIELDS']['DETAIL_PICTURE']):?>
	...
	<?if($arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'] == 'LEFT'):?>	
		...
		<div class="detailimage image-left col-md-4 col-sm-4 col-xs-12">
			<a href="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="fancybox a_block" title="<?=$atrTitle?>">
                 <div style="background-image:url(<?=$arResult['DETAIL_PICTURE']['SRC']?>); width: 100%; height: 185px; background-size: cover"></div>
			</a>
	...
	<?elseif($arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'] == 'RIGHT'):?>
		<div class="detailimage image-right col-md-4 col-sm-4 col-xs-12"><a href="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="fancybox" title=""><img src="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="img-responsive" title="" alt="" /></a></div>
	<?elseif($arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'] == 'TOP'):?>
		<script type="text/javascript">
		$(document).ready(function() {
			$('section.page-top').remove();
			$('<div class="row"><div class="col-md-12"><div class="detailimage image-head"><img src="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="img-responsive" title="" alt="" /></div></div></div>').insertBefore('.body > .main > .container > .row');
		});
		</script>
	<?else:?>
		<div class="detailimage image-wide"><a href="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="fancybox" title=""><img src="<?=$arResult['DETAIL_PICTURE']['SRC']?>" class="img-responsive" title="" alt="" /></a></div>
	<?endif;?>
<?endif;?>
...
<?if($arResult['GALLERY']):?>
	<div class="wraps">		
		<div class="title_block h4"><?=(strlen($arParams['T_GALLERY']) ? $arParams['T_GALLERY'] : GetMessage('T_GALLERY'))?></div>
...
<?php 
	//Ïîäêëþ÷åíèå ôîðìû
	//$APPLICATION->IncludeFile(SITE_DIR."bitrix/templates/aspro-stroy/include_areas/form1.php", Array(), Array("MODE"=>"html")); 
?>
	...
	<div class="order-block">
		<div class="row">
			<div class="col-md-4 col-sm-4 col-xs-5 valign">
				<span class="btn btn-default btn-lg" data-event="jqm" data-param-id="<?=CCache::$arIBlocks[SITE_ID]['aspro_stroy_form']['aspro_stroy_order_services'][0]?>" data-name="order_services" data-autoload-service="<?=$arResult['NAME']?>"><span><?=(strlen($arParams['S_ORDER_SERVICE']) ? $arParams['S_ORDER_SERVICE'] : GetMessage('S_ORDER_SERVICE'))?></span></span>
			</div>
