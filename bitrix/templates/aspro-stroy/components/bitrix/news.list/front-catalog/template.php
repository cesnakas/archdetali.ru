...
<?if($arResult['ITEMS']):?>
	...
	<div class="catalog item-views table front" style="display:none;">
		<div class="top_wrapper_block nomargin_bottom">
			...
			<div class="sclick-slider unstyled row" data-plugin-options='{"animation": "slide", "directionNav": true, "itemMargin":30, "controlNav" :true, "animationLoop": true, "slideshow": false, "counts": [<?=$countmd?>, <?=$countsm?>, <?=$countxs?>, <?=$countxsm?>]}'>
							...
							<div class="item<?=($bShowImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product">
								...
								<?if($bShowImage):?>
									<div class="image">
										...
										<?elseif($imageDetailSrc):?><a href="<?=$imageDetailSrc?>"  title="" class="img-inside fancybox" itemprop="url">
										<?endif;?>
											<img class="img-responsive" src="<?=$imageSrc?>" alt="" title="" itemprop="image" />
										<?if($bDetailLink):?></a>
								...
								<div class="text">
                                    ...
                                    <?php if(false && intval($arItem['DISPLAY_PROPERTIES']['SIZE']['VALUE']) > 0): ?>
                                        ...
                                        <div class="item-product-properties"><span class="item-icon price-icon"></span>
                                            <?php echo number_format($price_1, 2, ',', ' ');  ?>руб.</div>
                                        <?php if($srok > 1 AND $srok < 5): ?>
                                            <div class="item-product-properties"><span class="item-icon time-icon"></span>
                                                <?php echo $srok;?> мес€ца.</div>
                                        <?php endif; ?> 
                                        <?php if($srok == 1 OR $srok < 1): ?>
                                            <div class="item-product-properties"><span class="item-icon time-icon"></span>1 мес€ц.</div>
                                        <?php endif; ?>
                                        <?php if($srok > 4): ?>
                                            <div class="item-product-properties"><span class="item-icon time-icon"></span>
                                                <?php echo $srok;?> мес€цев.</div>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                    <?if(false && $isSizes){?>
