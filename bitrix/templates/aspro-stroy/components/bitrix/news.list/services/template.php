	...
	<?if($arResult['ITEMS']):?>
		<div class="items row">
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
                ...
                //09.10
				$resizeImageFile =CFile::ResizeImageGet(
					$arItem['FIELDS']['PREVIEW_PICTURE']['ID'], 
					Array("width" => 254, "height" => 190), 
				...
				<?ob_start();?>
					...
					<?endif;?>
					<?if($bDetailLink):?>
						<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="btn btn-default white"><span><?=GetMessage('TO_ALL')?></span></a>
					<?endif;?>
					...
					<?ob_start();?>
						<div class="image">
							...
							<?elseif($arItem['FIELDS']['DETAIL_PICTURE']):?>
								<a href="<?=$imageDetailSrc?>" alt="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-inside fancybox">
							<?endif;?>

						<div style="background-image:url(<?=$imageSrc?>); width: 100%; height: <?=$resizeImageFile['height']?>px; background-size: cover"></div>
