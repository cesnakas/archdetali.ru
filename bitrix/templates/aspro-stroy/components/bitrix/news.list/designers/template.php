...
<div class="item-views box <?=$arParams['VIEW_TYPE']?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> staff <?=($templateName = $component->{'__template'}->{'__name'})?>">
	<?// top pagination?>
	...
	<?if($arResult['ITEMS']):?>
		<div class="row items flexslider1">
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?ob_start();?>
						...
						<div class="row1">
							<?//if($bImage):?>
							<div class="image<?=($bImage ? '' : ' hidden-xs')?>">
						...
						<div class="row2">
							...
							<?// element experience?>
							<?if(strlen($arItem['DISPLAY_PROPERTIES']['EXPERT']['VALUE'])):?>
								<div class="post"><?=$arItem['DISPLAY_PROPERTIES']['EXPERT']['VALUE']?></div>
								<?unset($arItem['DISPLAY_PROPERTIES']['EXPERT']);?>
							<?endif;?>
						</div>
				...
				<?elseif($arParams['VIEW_TYPE'] == 'table'):?>
					<div class="col-md-6 col-sm-<?=floor(12 / round($arParams['COUNT_IN_LINE'] / 2))?>">
						<div class="item<?//=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
							<div class="row">
								<div class="col-md-12">
