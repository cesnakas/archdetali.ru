...
<?php
// Переменная формы. #FORM_ID_1# ЗАМЕНЯЕТ НА ФОРМУ
...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?>">
								...
								<div class="item review" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
									<div class="it">
										...
										<?if($arItem['PROPERTY_DOCUMENTS_VALUE']):?>
											<div class="row docs">
												<?$arItem['PROPERTY_DOCUMENTS_VALUE'] = is_array($arItem['PROPERTY_DOCUMENTS_VALUE']) ? $arItem['PROPERTY_DOCUMENTS_VALUE'] : array($arItem['PROPERTY_DOCUMENTS_VALUE']);?>
												<?foreach($arItem['PROPERTY_DOCUMENTS_VALUE'] as $docID):?>
													...
													<div class="col-md-6 item">
														...
														<div class="icons  <?=$arFile['TYPE']?>">
															<a href="<?=$arFile['SRC']?>" target="_blank" title="<?=$fileTitle?>"></a>
														</div>
														<div class="text">
		...
		<?if(in_array('LINK_STAFF', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_STAFF_VALUE']):?>
			<div class="wraps nomargin">
				<h4 class="title_block">Наши дизайнеры</h4>
				<?global $arrrFilter; $arrrFilter = array('ID' => $arElement['PROPERTY_LINK_STAFF_VALUE']);?>
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "designers", array(
					"IBLOCK_TYPE" => "aspro_stroy_content",
					"IBLOCK_ID" => CCache::$arIBlocks[SITE_ID]["aspro_stroy_content"]["aspro_stroy_staff"][0],
					"NEWS_COUNT" => "30",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "DESC",
					"SORT_BY2" => "",
		...
		<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_GOODS_VALUE']):?>
			<div class="wraps nomargin">
				...
				<div class="title_block h4"><?=(strlen($arParams['T_GOODS']) ? $arParams['T_GOODS'] : GetMessage('T_GOODS'))?></div>
		...
		<?if(in_array('LINK_SERVICES', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_SERVICES_VALUE']):?>
			<div class="wraps nomargin">
				...
				<h4 class="title_block"><?=(strlen($arParams['T_SERVICES']) ? $arParams['T_SERVICES'] : GetMessage('T_SERVICES'))?></h4>
