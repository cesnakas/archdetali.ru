...
<div class="item-views box <?=$arParams['VIEW_TYPE']?> <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
	<?// top pagination?>
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?if($arParams['SHOW_SECTION_PREVIEW_DESCRIPTION'] == 'Y'):?>
								...
								<?if(strlen($arSection['NAME'])):?>
									<h3 class="title_block"><?=$arSection['NAME']?></h3>
								<?endif;?>
											...
											<div class="properties">
												<?foreach($arItem['DISPLAY_PROPERTIES'] as $PCODE => $arProperty):?>
													<div class="property">
														...
														<?if($PCODE == 'SITE'):?>
															<!--noindex-->
															<?=str_replace("href=", "rel='nofollow' target='_blank' class='color_link' href=", $val);?>
															<!--/noindex-->
														<?elseif($PCODE == 'EMAIL'):?>
															<a href="mailto:<?=$val?>" class="color_link"><?=$val?></a>
														<?else:?>
															<?=$val?>
														<?endif;?>
													</div>
