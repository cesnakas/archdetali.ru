...
$arElement = CCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arItemFilter, false, false, array('ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PROPERTY_LINK_PROJECTS', 'PROPERTY_LINK_GOODS', 'PROPERTY_LINK_REVIEWS', 'PROPERTY_LINK_STAFF', 'PROPERTY_LINK_STUDY', 'PROPERTY_LINK_SERVICES'));
?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?>">
		...
		<?if(in_array('LINK_GOODS', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_GOODS_VALUE']):?>
			<div class="wraps nomargin">
				<h4 class="title_block"><?=(strlen($arParams['T_GOODS']) ? $arParams['T_GOODS'] : GetMessage('T_GOODS'))?></h4>
		...
		<?if(in_array('LINK_STUDY', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_STUDY_VALUE']):?>
			<div class="wraps nomargin">				
				<h4 class="title_block"><?=(strlen($arParams['T_STUDY']) ? $arParams['T_STUDY'] : GetMessage('T_STUDY'))?></h4>
				...
				<?$APPLICATION->IncludeComponent(
					...
					array(
						"IBLOCK_TYPE" => "aspro_stroy_catalog",
						"IBLOCK_ID" => CCache::$arIBlocks[SITE_ID]["aspro_stroy_catalog"]["aspro_stroy_study"][0],
						"NEWS_COUNT" => "20",
						"SORT_BY1" => "ACTIVE_FROM",
						"SORT_ORDER1" => "DESC",
						"SORT_BY2" => "SORT",
