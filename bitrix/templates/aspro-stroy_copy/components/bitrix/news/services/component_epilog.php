...
if ( ($arParams["IBLOCK_ID"] == 60) && ($APPLICATION->GetPageProperty('description')!=$ttt) && (!(empty($ttt))) )  {
	...
	if ($important_description) {
		...
		$APPLICATION->SetPageProperty("description",$APPLICATION->GetTitle().'. Блог. Разработка дизайна интерьеров в Москве по привлекательным ценам от компании "Arch Detali".');
	}
	if ($arParams["IBLOCK_ID"] == 75) {
		$APPLICATION->SetPageProperty("description",$APPLICATION->GetTitle().'. Разработка дизайна интерьеров в Москве по привлекательным ценам от компании "Arch Detali".');
	}
