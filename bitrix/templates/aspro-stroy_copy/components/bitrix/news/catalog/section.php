...
<?else:?>
	...
	<?$frame->end();?>
    <?if($arSubSections):?>
    	<?//$interes_sections = getSectionList(Array('name'=>'asc'), array('ACTIVE'=>'Y','IBLOCK_ID'=>$arParams["IBLOCK_ID"],'ID'=>$category['UF_SECTIONS']), false, array('ID','NAME','SECTION_PAGE_URL', 'PICTURE', 'UF_TEXT_INTERES'));
		//print_r($arSection['ID']);
		$arFilter = Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ID"=>$arSection['ID'], "ACTIVE"=>"Y");
		$SectList = CIBlockSection::GetList(array(), $arFilter ,false, array("UF_TITLE_SECT"));
        while ($SectListGet = $SectList->GetNext())
        {
            //print_r($SectListGet);
            $arSubSectionsGet = $SectListGet["UF_TITLE_SECT"];
        }
		?>
        <?// sections list?>
        <?$APPLICATION->IncludeComponent(
            ...
            Array(
                ...
                "DISPLAY_PANEL"	=>	$arParams["DISPLAY_PANEL"],
                "SET_TITLE"	=>	"N",
                "arSubSectionsGet"	=>	$arSubSectionsGet,
                "SET_STATUS_404" => "N",
