									...
									<?foreach($arResult['GALLERY'] as $i => $arPhoto):?>
										<li class="col-md-1 col-sm-1 item">
											<a href="<?=$arPhoto['DETAIL']['SRC']?>" class="fancybox" rel="gallery" target="_blank" title="<?=$arPhoto['TITLE']?>">
												<img src="<?=$arPhoto['PREVIEW']['src']?>" class="img-responsive inline" title="" alt="" />
												<span class="zoom">
													<i class="fa"></i>
												</span>
											</a>
							...
							<table class="props_table">
								<?foreach($arResult['CHARACTERISTICS'] as $arProp):?>
									<tr class="char">
										...
										<td class="char_value">
											<span>
												<?if($arProp['NAME'] == '����') { $arProp['DISPLAY_VALUE'] = str_replace('<a ','<a rel="nofollow" ',$arProp['DISPLAY_VALUE']);} ?>
