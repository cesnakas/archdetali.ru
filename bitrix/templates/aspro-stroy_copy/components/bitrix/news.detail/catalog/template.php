...
$arWaterMark = Array(
    array(
        ...
        "type" => "image",
        "size" => "real",
//        "file" => $_SERVER["DOCUMENT_ROOT"].'/upload/resize_cache/water/bf5/250_127_2/bf559343ad0219b93ef325ea7a1ce6f3.png',
        "file" => $_SERVER["DOCUMENT_ROOT"].'/bitrix/templates/aspro-stroy_copy/img/watermark_2.png',
        "fill" => "exact",
    )
);
$file = CFile::MakeFileArray($arWaterMark[0]['file']);
///$file2 = CFile::SaveFile($file, 'water', false, false, 'water', true);

$arWaterMarkForThumb = CFile::ResizeImageGet(
    $file,
											...
											<ul class="slides items">
												...
												<?foreach($arResult['GALLERY'] as $i => $arPhoto):?>
													<li class="col-md-12 col-sm-12 item">
														<?
														$arFileTmpDetalePic = array();
														$arFileTmpPreviwPic = array();
														$arFileTmpDetalePic = CFile::ResizeImageGet(
															$arPhoto['DETAIL'],
														...
														$arFileTmpPreviwPic = CFile::ResizeImageGet(
															$arPhoto['DETAIL'],
														...
														?>
														<a href="<?=$arFileTmpDetalePic['src']?>" class="fancybox" rel="gallery" target="_blank">
															<img src="<?=$arFileTmpPreviwPic['src']?>" class="img-responsive inline" title="<?=$arResult["NAME"]?> – «Arch Detali»"  alt="<?=$arResult["NAME"]?> – «Arch Detali»" itemprop="image" />
															<span class="zoom">
																<i></i>
															</span>
														</a>
					...
					<?foreach($arResult['PLANIROVKA'] as $arFile){?>
						<div class="col-md-<?=$col;?> galery">
							<div class="plans_block border_block item">
								<div class="img_block">
									<a href="<?=$arFile['DETAIL']['SRC']?>" class="fancybox blink" rel="plans" target="_blank">
										<img class="img-responsive" src="<?=$arFile["PREVIEW"]["src"]?>" alt=""/>
										<span class="zoom">
											<i></i>
										</span>
									</a>
													...
													<td class="char_value">
														<span>
															...
															<?else:?>
                                                                ...
                                                                <?php
                                                                    if(strpos($keyProp, 'SIZE') !== false){
                                                                        echo GetMessage('SIZE_UNIT');
                                                                    }
                                                                    if(strpos($keyProp, 'PRICE_') !== false){
				...
				<?if($arResult["GALLERY_BIG"]){?>
					...
					<div class="item-views portfolio front">
						<div class="row" itemscope itemtype="http://schema.org/ItemList">
							<?foreach($arResult["GALLERY_BIG"] as $key => $arBigItem){?>
								<div class="col-md-2 col-sm-4 col-xs-6">
									<div class="item wline">
										...
										?>
										<a href="<?=$arFileTmpDetalePic["src"]?>" alt="<?=$arResult["NAME"]?> – ôîòî <?=(2+$key)?> – «Arch Detali»"  title="<?=$arResult["NAME"]?> – ôîòî <?=(2+$key)?> – «Arch Detali»"  class="dark_block_animate fancybox_ext" rel="big_gallery"></a>
										<div class="img_block scale_block_animate" alt="<?=$arResult["NAME"]?> – ôîòî <?=(2+$key)?> – «Arch Detali»"  style="background-image: url('<?=$arFileTmpPreviwPic["src"];?>');"></div>
									</div>
								</div>
							<?}?>
						</div>
