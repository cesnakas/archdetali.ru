<?
$MESS['S_ASK_QUESTION'] = 'Задать вопрос';
$MESS['S_ORDER_SERVICE'] = 'Заказать услугу';
$MESS['S_ORDER_STUDY'] = 'Заказать курс';
$MESS['T_DOCS'] = 'Документы';
$MESS['T_GALLERY'] = 'Галерея';
$MESS['T_PROJECTS'] = 'Выполненные проекты';
$MESS['T_REVIEWS'] = 'Отзывы клиентов';
$MESS['T_STAFF1'] = 'Сотрудник';
$MESS['T_STAFF2'] = 'Сотрудники';
$MESS['T_GOODS'] = 'Проекты, в которых использовалась услуга';
$MESS['T_SERVICES'] = 'Услуги';
$MESS['T_CHARACTERISTICS'] = 'Характеристики';
$MESS["T_VIDEO"] = "Видео";
$MESS["T_PROFIT"] = "Почему выбирают нас";
$MESS['SHARE_TEXT'] = 'Поделиться ссылкой:';
?>